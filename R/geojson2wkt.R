...
geojson2wkt <- function(obj, fmt = 16, third = "z", use_cpp = FALSE, ...) {
  UseMethod("geojson2wkt")
}

#' @export
geojson2wkt.default <- function(obj, fmt = 16, third = "z", use_cpp = FALSE, ...) {
  stop("No 'geojson2wkt' method for ",
    paste0(class(obj), collapse = ", "), call. = FALSE)
}

#' @export
geojson2wkt.character <- function(obj, fmt = 16, third = "z", use_cpp = FALSE, ...) {
  geojson2wkt(jsonlite::fromJSON(obj, ...), fmt, third)
}

#' @export
geojson2wkt.json <- function(obj, fmt = 16, third = "z", use_cpp = FALSE, ...) {
  geojson2wkt(jsonlite::fromJSON(obj, ...), fmt, third)
}

#' @export
geojson2wkt.list <- function(obj, fmt = 16, third = "z", use_cpp = FALSE, ...) {
  ...
  switch(
    tolower(obj$type),
    point = {
      if (use_cpp) wk_dump_point(obj, fmt, third) else dump_point(obj, fmt, third)
    },
    multipoint = {
      if (use_cpp) wk_dump_multipoint(obj, fmt, third) else dump_multipoint(obj, fmt, third)
    },
    linestring = {
      if (use_cpp) wk_dump_linestring(obj, fmt, third) else dump_linestring(obj, fmt, third)
    },
    multilinestring = {
      if (use_cpp) wk_dump_multilinestring(obj, fmt, third) else dump_multilinestring(obj, fmt, third)
    },
    polygon = {
      if (use_cpp) wk_dump_polygon(obj, fmt, third) else dump_polygon(obj, fmt, third)
    },
