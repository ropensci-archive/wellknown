...
test_that("convert point works", {
  ...
  a <- suppressWarnings(geojson2wkt(point))
...
test_that("convert multipoint works", {
  ...
  b <- suppressWarnings(geojson2wkt(mp))
...
test_that("convert linestring works", {
  ...
  cc <- suppressWarnings(geojson2wkt(st))
  expect_is(cc, "character")
  expect_match(cc, "LINESTRING")
  expect_equal(suppressWarnings(geojson2wkt(st, 0)), "LINESTRING (0 0, 2 1, 4 2, 5 4)")
  expect_equal(length(strsplit(suppressWarnings(geojson2wkt(st, 1)), "\\.")[[1]]), 9)
})
...
test_that("convert multilinestring works", {
  ...
  d <- suppressWarnings(geojson2wkt(multist))
  expect_is(d, "character")
  expect_match(d, "LINESTRING")
  expect_match(d, "MULTILINESTRING")
  expect_equal(suppressWarnings(geojson2wkt(multist, 0)), "MULTILINESTRING ((0 -1, -2 -3, -4 -5), (1.6600 -31.5000, 10.9999 3.0000, 10.9000 1.1000, 0.0000 0.0000))")
})
...
test_that("convert polygon works", {
  ...
  e <- suppressWarnings(geojson2wkt(poly))
  expect_is(e, "character")
  expect_match(e, "POLYGON")
  expect_equal(suppressWarnings(geojson2wkt(poly, 0)), "POLYGON ((100.001 0.001, 101.100 0.001, 101.001 1.001, 100.001 0.001), (100.201 0.201, 100.801 0.201, 100.801 0.801, 100.201 0.201))")
})
...
test_that("convert multipolygon works", {
  ...
  f <- suppressWarnings(geojson2wkt(mpoly2, fmt=0))
...
test_that("convert geometrycollection works", {
  ...
  g <- suppressWarnings(geojson2wkt(gmcoll, fmt = 0))
...
test_that("point - fails well", {
  ...
  expect_error(
    suppressWarnings(geojson2wkt(mp)),
...
test_that("multipoint - throws warning with old format", {
  mp <- list(
    type = 'MultiPoint',
    coordinates = matrix(c(100, 101, 3.14, 3.101, 2.1, 2.18), ncol = 2)
  )
  expect_is(suppressWarnings(geojson2wkt(mp)), "character")
  expect_warning(geojson2wkt(mp), "using old format")
})

test_that("linestring - throws warning with old format", {
  ll <- list(type = 'LineString',
             coordinates = matrix(c(0.0, 2.0, 4.0, 5.0,
                                    0.0, 1.0, 2.0, 4.0), ncol = 2))
  expect_is(suppressWarnings(geojson2wkt(ll)), "character")
  expect_warning(geojson2wkt(ll), "using old format")
})

test_that("multilinestring - throws warning with old format", {
  ll <- list(type = 'MultiLineString',
             coordinates = list(
               matrix(c(0, -2, -4, -1, -3, -5), ncol = 2),
               matrix(c(1.66, 10.9999, 10.9, 0, -31.5, 3.0, 1.1, 0), ncol = 2)
             ))
...
test_that("polygon - throws warning with old format", {
  ll <- list(type = 'Polygon',
            coordinates = list(
              matrix(c(100.001, 101.1, 101.001, 100.001, 0.001, 0.001, 1.001, 0.001),
                     ncol = 2),
              matrix(c(100.201, 100.801, 100.801, 100.201, 0.201, 0.201, 0.801, 0.201),
...
test_that("multipolygon - throws warning with old format", {
  ll <- list(type = "MultiPolygon",
             coordinates = list(
               list(
                 matrix(c(100, 101, 101, 100, 0.001, 0.001, 1.001, 0.001), ncol = 2),
                 matrix(c(100.2, 100.8, 100.8, 100.2, 0.2, 0.2, 0.8, 0.2), ncol = 2)
...
test_that("multipoint - fails well with bad input", {
  ...
  expect_error(suppressWarnings(geojson2wkt(mp)), "expecting a matrix in 'coordinates', got a list")
})
...
test_that("linestring - fails well with bad input", {
  ...
  expect_error(suppressWarnings(geojson2wkt(st)),
               "expecting a matrix")
})
...
test_that("multilinestring - fails well with bad input", {
  ...
  expect_error(suppressWarnings(geojson2wkt(multist)), "expecting matrices for all 'coordinates' elements")
})
...
test_that("polygon - fails well with bad input", {
  ...
  expect_error(suppressWarnings(geojson2wkt(poly)), "expecting matrices for all 'coordinates' elements")
})
...
test_that("multipolygon - fails well with bad input", {
  ...
  expect_error(suppressWarnings(geojson2wkt(mpoly2)), "")
})
...
test_that("geometrycollection - fails well with bad input", {
  ...
  expect_error(suppressWarnings(geojson2wkt(gmcoll)), "")
})
